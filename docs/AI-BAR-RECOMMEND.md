...
基于用户饮酒偏好画像（SBTI）+ 实时对话意图，通过腾讯混元大模型智能推荐酒馆。
...
用户首次点击 AI 悬浮窗（首页/酒百科页）
  ...
  进入 AI 对话页面
        ↓
  用户输入消息 → aiChat() 单函数处理
  （一次混元调用，prompt 内置意图识别 + 推荐/知识/闲聊能力）
        ↓
  返回结果 → 前端按 intent 渲染
        ↓
  异步：离线分析对话 → 微调 SBTI
...
| AI 核心架构 | **单函数 + Prompt 内置意图识别** | 一次混元调用完成意图判断 + 回复生成，简洁高效 |
...
async function aiChat(currentUser, payload) {
  ...
  // 6. 调用混元
  const aiRaw = await callHunyuan(systemPrompt, [
  ...
  // 10. 异步微调
  analyzeSbti(session._id, currentUser._id).catch(console.error);
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `user_id` | String | 是 | 关联 `user_profile._id`，一对一 |
...
| `preferred_areas` | String[] | 否 | 问卷不采集，由离线微调提取 |
...
> **目标**：数据层就绪，混元 API 可调通  
> **前置**：无  
> **验收**：3 个集合已创建，酒馆有种子数据，`callHunyuan()` 能正常返回

| # | 任务 | 产出 |
|---|------|------|
| 1.1 | 申请腾讯混元 API Key | SecretId + SecretKey |
| 1.2 | 云数据库创建 `user_sbti` + `bar_info` + `ai_chat_session`，建索引 | 集合就绪 |
| 1.3 | 后端 `COLLECTIONS` 注册 3 个新集合 | `index.js` 更新 |
| 1.4 | 配置云函数环境变量 | SecretId / SecretKey / Model |
| 1.5 | 后端：`callHunyuan()` 封装（含腾讯云 API v3 签名） | 混元调用函数 |
| 1.6 | 后端：`safeParseAIJson()` 容错解析 | JSON 解析函数 |
| 1.7 | 手动录入 10~20 家酒馆种子数据 | `bar_info` 有数据 |
| 1.8 | 测试：云函数中调用混元 → 正常返回 | API 调通 |
...
| # | 任务 | 产出 |
|---|------|------|
| 2.1 | 后端：`sbti.get` / `sbti.init` / `sbti.update` 接口 | 3 个 action | ✅ 已完成 |
| 2.2 | 前端：问卷页 `pages/ai/sbti-survey`（支持 `mode=redo`） | 页面四件套 | ✅ 已完成 |
| 2.3 | 前端：画像展示页 `pages/ai/sbti-profile` | 页面四件套 | ⬜ 待开发 |
| 2.4 | 前端：「我的」页面新增「我的饮酒画像」入口 | 修改 profile/index | ⬜ 待开发 |
| 2.5 | `app.json` 注册两个新页面 | 路由生效 | ✅ 已完成 |
| 2.6 | 测试：问卷 → 画像 → 重测 → 数据库验证 | 全流程走通 | ⬜ 待测试 |

**更新日志（2026-05-09）**：
- ✅ 恢复问卷页真实提交功能，调用 `sbti.init` 云函数
- ✅ 云函数已支持通过 `openid` 查询和创建用户画像
- ✅ 用户提交问卷后，数据会真实保存到云数据库 `user_sbti` 集合
...
> **前置**：FT-1（混元可调用） + FT-2（SBTI 有数据）  
> **验收**：发消息 → AI 正确回复 → 推荐带酒馆卡片 → 卡片可点击看详情

| # | 任务 | 产出 |
|---|------|------|
| 3.1 | 后端：`aiChat()` 主函数（完整流程：加载数据 → 组装 prompt → 调混元 → 解析 → 存 session） | 核心 action |
...
| # | 任务 | 产出 |
|---|------|------|
| 5.1 | 后端：`analyzeSbti()` 完整实现（调混元 + 解析 + 合并标签） | 异步函数 |
...
| 风险 | 应对 |
|------|------|
| 混元 API 响应慢（>5s） | loading 动画；云函数超时 30s |
| 混元返回非标准 JSON | `safeParseAIJson()` 容错 + fallback |
| 酒馆数据太少 | prompt 指示"推荐最接近的" |
| 意图识别偏差 | prompt 中给出明确规则；观察日志持续优化 |
| SBTI 微调不稳定 | 仅 `confidence=high` 更新 |
| 调用成本 | `hunyuan-lite`；限 10 轮/会话 |
...
- [x] 需求确认
- [x] 方案设计
- [ ] **FT-1：基础设施搭建** ← 下一步（需申请混元 API Key）
- [ ] **FT-2：SBTI 画像全流程** ← 部分完成（问卷页已就绪）
- [ ] FT-3：AI 对话核心 + 酒馆详情
- [ ] FT-4：悬浮窗 + 入口串联
- [ ] FT-5：SBTI 离线微调
- [ ] FT-6：收尾打磨
...
- ✅ 问卷页面开发完成，支持多选和单选题型
- ✅ 恢复真实提交功能，数据保存到云数据库
- ✅ 云函数 `sbti.init` 接口已实现并测试通过
- 📝 待开发：画像展示页、AI 对话页、酒馆推荐逻辑
